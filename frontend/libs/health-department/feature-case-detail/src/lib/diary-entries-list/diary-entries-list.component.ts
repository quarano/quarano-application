...
export class DiaryEntriesListComponent implements OnInit {
  ...
  private convertToDiaryEntries(groupedDiaryEntries: Dictionary<any[]>): TrackedCaseDiaryEntry[] {
    ...
    for (const [date, diaryEntries] of Object.entries(groupedDiaryEntries)) {
      const entryMorning = diaryEntries.filter((diaryEntry: TrackedCaseDiaryEntryDto) => diaryEntry.slot.timeOfDay === this.MORNING)?.[0];
      const entryEvening = diaryEntries.filter((diaryEntry: TrackedCaseDiaryEntryDto) => diaryEntry.slot.timeOfDay === this.EVENING)?.[0];
