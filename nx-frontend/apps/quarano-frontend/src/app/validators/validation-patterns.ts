...
export class ValidationPattern {
  ...
  private static initialize() {
    ...
    this._patterns.set(
      VALIDATION_PATTERNS.integerUnsigned,
      {
        pattern: '^[0-9]+$',
    ...
    this._patterns.set(VALIDATION_PATTERNS.street,
      {
        ...
        errorMessage: 'Bitte geben Sie eine gültige Straße an. Diese darf Buchstaben, Zahlen, Leerzeichen, ' +
          'Punkte und Bindestriche enthalten.'
      });
    this._patterns.set(VALIDATION_PATTERNS.houseNumber,
      {
        pattern: `[${characterSet}0-9\\s\\.\\-*\\/]{0,15}`,
        errorMessage: 'Bitte geben Sie eine gültige Hausnummer an. Diese darf maximal 15 Zeichen lang sein und ' +
          'nur aus Buchstaben, Zahlen, Punkten, Binde- und Schrägstrichen bestehen'
      });
    this._patterns.set(VALIDATION_PATTERNS.name,
      {
        pattern: `([${characterSet}\\s\\-]*)`,
        errorMessage: 'Bitte geben Sie einen gültigen Namen ein. Es sind nur Buchstaben, Leerzeichen und Bindestriche erlaubt'
      });
    this._patterns.set(VALIDATION_PATTERNS.username,
      {
        pattern: `([${characterSet}0-9\\-\\_]*)`,
        errorMessage: 'Bitte geben Sie einen gültigen Benutzernamen ein! Dieser kann Buchstaben, Zahlen, Binde- oder Unterstriche enthalten'
      });
    this._patterns.set(VALIDATION_PATTERNS.phoneNumber,
      {
        pattern: '^[\\+\\(\\)\\s\\-0-9]*?$',
        errorMessage: 'Bitte geben Sie eine gültige Telefonnummer an! Diese kann Zahlen, +, -, Klammern oder Leerzeichen enthalten'
      });
